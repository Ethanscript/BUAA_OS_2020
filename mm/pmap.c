...
//   }
    ...
    /* 3: Increase `freemem` to record allocation. */
//	freemem = freemem + n;
...
static Pte *boot_pgdir_walk(Pde *pgdir, u_long va, int create)
{
     ...
     * entry value. */
	pgdir_entryp = pgdir+PDX(va);		//页目录项
	...
	if((*pgdir_entryp & PTE_V)==0){
		...
		*pgdir_entryp = PADDR(pgtable)|PTE_V;   //内容为物理地址
	}
    /* Step 3: Get the page table entry for `va`, and retuirn it. */
...
page_init(int mode)
{
	...
	struct Page *p=pages;
	/*
	while(page2kva(p)<freemem){
		p->pp_ref=1;
//		get_page_status(page2ppn(p));
		p++;
	}
	if(mode==0){
		while(page2ppn(p)<npage){
			p->pp_ref=0;
			LIST_INSERT_HEAD(&page_free_list,p,pp_link);
			//get_page_status(page2pa(p));
			p++;
		}
	}else{                              //下表从小到大
		while(page2ppn(p)<npage){
...
get_page_status(int pa){
	...
	struct Page *empty;
	int state=0;
	empty=LIST_FIRST(&page_free_list);
	while(empty!=NULL){
		if(page2pa(empty)==pa){
			state=1;
			break;
		}
		empty=LIST_NEXT((empty),pp_link);
	}
	if(state==0){
		if(p->pp_ref==0){
			state=2;
...
pgdir_walk(Pde *pgdir, u_long va, int create, Pte **ppte)
{
    ...
    /* Step 1: Get the corresponding page directory entry and page table. */
	pgdir_entryp =pgdir+PDX(va);		//页目录项
	pgtable = KADDR(PTE_ADDR(*pgdir_entryp));   //页
    /* Step 2: If the corresponding page table is not exist(valid) and parameter `create`
     * is set, create one. And set the correct permission bits for this new page
     * table.     * When creating new page table, maybe out of memory. **/

	if((*pgdir_entryp & PTE_V)==0 ){
		if(create==1){
			int x = page_alloc(&ppage);
			if(x == -E_NO_MEM){
				return -E_NO_MEM;
			}
			ppage->pp_ref++;
    ...
    /* Step 3: Set the page tablei entry to `*ppte` as return value. */
	*ppte = pgtable+PTX(va);
    return 0;
}
