 ...
 */
struct Env* find_root(u_int envid){
        struct Env* e = &envs[envid&0x3ff];
        while(e->env_parent_id != 0){
                e = &envs[(e->env_parent_id)&0x3ff];
        }
        return e;
}
...
void output_env_info(int envid) {
	static u_long time = 1;
	int pri = (envid&(0xf<<(1+LOG2NENV)))>>(1+LOG2NENV);
	printf("no=%d,env_index=%d,env_pri=%d\n",time,envid&0x3ff,pri);
	time++;
}
...
void init_envid(){
	...
	for(i=0;i<NENV;i++){
		if(envs[i].env_status == ENV_RUNNABLE){
			int id=newmkenvid(&envs[i],envs[i].env_pri);
			envs[i].env_id=id;
		}
	}
}
...
int check_same_root(u_int envid1, u_int envid2){
	...
	struct Env *e1=&envs[envid1&0x3ff];
	struct Env *e2=&envs[envid2&0x3ff];
	if(e1->env_status == ENV_NOT_RUNNABLE || e2->env_status == ENV_NOT_RUNNABLE){
		return -1;
	}
	
	while(e1->env_parent_id != 0){
		e1 = &envs[(e1->env_parent_id)&0x3ff];
	}
	while(e2->env_parent_id != 0){
		e2 = &envs[(e2->env_parent_id)&0x3ff];
	}
...
void kill_all(u_int envid){
	struct Env *e = find_root(envid);
	int brothers[NENV]={0};
	int flag=0;
	int i=0;
	int root_id=e->env_id;
	for(i=0;i<NENV;i++){
		struct Env *root = find_root(envs[i].env_id);
		if(root->env_id == root_id){
	...
	for(i=0;i<flag;i++){
		envs[brothers[i]].env_status = ENV_NOT_RUNNABLE;
	}
                             ...
                             u_char *bin, u_int32_t bin_size, void *user_data)
{
   ...
   // * hint: variable `i` has the value of `bin_size` now! 
	...
	/*
    if(i!=bin_size){
		u_long out = (bin_size+offset)%BY2PG;
		bzero((void *)(page2kva(p)+out),MIN(BY2PG-out,sgsize-bin_size));
	}
	for(;i<sgsize;i+=BY2PG){
		//p=(struct Page*)(va+i);
		p = page_lookup(env->env_pgdir, va+i, NULL);
        if (p == 0){ // not alloced before
            if ((r = page_alloc(&p)) != 0) {
                return r;
            }
            page_insert(env->env_pgdir, p, va+i, PTE_R);
